...
TableModel::TableModel(QObject *parent /*= Q_NULLPTR*/, bool checkable)
...
void TableModel::resize(std::size_t rows, std::size_t columns)
{
...
QVariant& TableModel::at(std::size_t row, std::size_t column)
{
...
void TableModel::setCheckState(int row, Qt::CheckState state)
{
	setData(index(row, 0), state, Qt::CheckStateRole);
}

Qt::CheckState TableModel::checkState(int row) const
{
...
void TableModel::startModelBuilding(qsizetype columns, qsizetype rows)
{
	beginResetModel();
	resize(rows, columns);
...
void TableModel::endModelBuilding()
{
...
QVariant TableModel::getHorizontalHeader(int index) const
{
	return headerData(index, Qt::Horizontal, Qt::DisplayRole);
}

void TableModel::setHorizontalHeader(int index, QVariant &value)
{
	setHeaderData(index, Qt::Horizontal, value, Qt::DisplayRole);
}
void TableModel::setHorizontalHeader(int index, const QString& value)
{
...
QString TableModel::createCSVString(const QChar separatorChar) const
{
...
void TableModel::copyToClipboard(const QChar separatorChar) const
{
...
{
	setStatus(TableModel::Status::OutDated);
}

void TableModel::setStatus(TableModel::Status status)
{
	if (status != m_status)
	{
		m_status = status;
...
void TableModel::setHeaderStatus(TableModel::Status status)
{
...
TableModel::Status TableModel::status()
{
	return m_status;
}
...
#ifdef TESTING
bool TableItemModelTest()
{
	 QSharedPointer<TableItemModel> model(new TableItemModel(nullptr,true));
...
static bool TableItemModelTestPerformed = TableItemModelTest();
#endif