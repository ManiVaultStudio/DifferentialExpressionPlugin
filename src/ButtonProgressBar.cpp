...

{
    ...
    showStatus(TableModel::Status::Undefined);
	delete layout();
    setLayout(newLayout);
}
...
void ButtonProgressBar::showStatus(TableModel::Status status)
{
	switch(status)
	{
		case TableModel::Status::Undefined:
			{
				_button->hide();
        ...
        case TableModel::Status::OutDated:
	        {
		        _progressBar->hide();
#ifdef _DEBUG
		...
		case TableModel::Status::Updating:
	        {
            _button->hide();
		...
		case TableModel::Status::UpToDate:
	        {
            _button->hide();
